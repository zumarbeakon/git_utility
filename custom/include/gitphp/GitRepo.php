	 ...
	 function runCommand($command)
{
	 ...
	 * @return  string
	 */  
	protected function runCommand2($command)
	{
		$descriptorspec = [
			1 => ['pipe', 'w'],
			2 => ['pipe', 'w'],
	...
	public function commit($message = "", $commitAll = true)
	{
		...
		return $this->run($sugar_config['GITHome']." git commit -m " . escapeshellarg($message));
	}
	...
	public function push($remote = "", $branch = "")
	{
		...
		return $this->run("git push $remote $branch");
	}
	...
	{
		return $this->run("git pull origin dev");
	}
