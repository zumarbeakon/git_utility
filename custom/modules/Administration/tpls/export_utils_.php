		...
		foreach ($val as $key => $value)
		{
            ...
            if (isset($focus->field_name_map[$fieldNameMapKey])  && $focus->field_name_map[$fieldNameMapKey]['type'])
            {
                ...
                switch ($fieldType)
                {
					...
					/* Yura 08.02.2018 start */
					// if field is actual part of relate field - convert it to actual name
					case 'id':
						foreach ($focus->field_name_map as $field_info){
							if (!empty($value)){
								if (isset($field_info['id_name']) && $field_info['id_name'] == $fieldNameMapKey){
									$parent_table = isset($field_info['table']) ? $field_info['table'] : strtolower($field_info['module']);
									$parent_field_sel = isset($field_info['rname']) ? $field_info['rname'] : 'name';
									$parent_field_db = $parent_field_sel;

									if ($parent_table == 'users' && $parent_field_sel == 'name')
										$parent_field_sel = 'CONCAT(first_name, " ", last_name) as '.$parent_field_db;

									if (!isset($GLOBALS['CSV_Export_datatable_'.$parent_table])){
										global $db;
										$sql = 'SELECT id, '.$parent_field_sel.' FROM `'.$parent_table.'` WHERE deleted = 0';
										$res = $db->query($sql);
										$result = array();
										while($row = $db->fetchByAssoc($res)){
											$result[$row['id']] = $row[$parent_field_db];
										}
										$GLOBALS['CSV_Export_datatable_'.$parent_table] = $result;
									}
									$value = $GLOBALS['CSV_Export_datatable_'.$parent_table][$value];
								}
							}
						}
					break;
             ...
             switch ($type) {
                ...
                case "phone":
                 ...
                 case "varchar":
                     ...
                     break;
                case "url":
                     $returnContent .= '"https://beakon.com.au/",';
                     break;
